...
function calculateMicroBench(benchmark, data) {
  ...
  const arraySum = rawData.reduce(function(pre,cur) {return pre + cur});
  const arrayAverage = arraySum/rawData.length;
...
class Runner {
          ...
          p.then(results => {
            ...
            return this._runSuite(data.suite).then(microBench => {
              results[data.name] = microBench;
	            //console.log("first result is " + JSON.stringify(results));
              return results;
            });
          }),
        Promise.resolve({})
  ...
  _loopTask(benchmark, ctx) {
    ...
    //console.log("loopTask benchmark is " + JSON.stringify(benchmark));
